...
#     #代理池
    ...
    def pro(self,url,uid,pid):
        ...
        for astr in str:
            ...
            rrr = requests.get(url_img.format(astr[0]), headers=headers2,proxies=proxies)
            rrr2 = requests.get(url_img2.format(astr[0]), headers=headers2,proxies=proxies)
            try:
                if rrr.status_code == 200:
                    with open('/Users/%s/%s.png' % (uid, i), 'wb+') as f:  ##默认的村塾路径
                        # if rrr.status_code==200:
...
if __name__ == '__main__':
    ...
    for uid in uids:
        ...
        url = 'https://www.pixiv.net/ajax/user/{uid}/profile/illusts?{pid}&work_category=illustManga&is_first_page=1&lang=zh'.format(
            uid=uid, pid=pids)
        pool.apply_async(gevent().pro, (url, uid, pids,))
    pool.close()
    pool.join()